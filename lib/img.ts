...
module img {
	...
	export const process = (options: svgOptions): Promise<string> => new Promise (async (resolve, reject) => {
		...
		else { resolve(`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xhtml="http://www.w3.org/1999/xhtml" width="${ width }" height="${ height }"><foreignObject width="${ width }" height="${ height }"><style>span { font-family: 'Century Gothic', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; line-height: 1.5rem; }</style><div xmlns="http://www.w3.org/1999/xhtml" style="display:flex; flex-direction:row; justify-content:flex-start; align-items:center; width:100%; height:100%; border-radius:${ bRadius }px; background-color:${ bgColour }; color:${ textColour }; padding:0 14px; box-sizing:border-box; overflow:clip;"><div style="display:flex; height: fit-content; width: fit-content;"><img src="data:${ mimetype };base64,${buffer.toString('base64')}" alt="Cover art" style="border: 1.6px solid ${ accentColour }; border-radius:${ aRadius }px; background-color:${ accentColour }" width="100px" height="100px" /></div><div style="display:flex; flex-direction:column; padding-left:14px;"><span style="font-size: 20px; font-weight: bold; padding-bottom: 6px; border-bottom: 1.6px solid ${ accentColour };">${ title }</span><span style="font-size:16px; font-weight:normal; margin-top:4px;">${ artist } - ${ album }</span></div></div></foreignObject></svg>`); }
	});
}
